...
endpoint = "dbtest.cl7fmg364enb.us-east-2.rds.amazonaws.com"
port = "3306"
user = "admin"
password = "xyz12345"
database = "app"
...
def kmeansClustering():
   ...
   # sql = "select {},{} from app.titanic3 WHERE(app.titanic3.{} IS NOT NULL AND app.titanic3.{} IS NOT NULL );".format(column1,column2,column1,column2)
    sql="select pclass,survived from app.titanic3"
    print(sql)
    starttime = time.time()
...
def DistanceCentroidPoints():
    ...
    cursor = connectDb()
    sql = "select '" + column1 + " ', ' "+ column2 +"' from app.titanic3"
    cursor.execute(sql)
    result = cursor.fetchall()
    cursor.close()
...
def question8():
    ...
    sql = "select "+col1+", "+col2+" from minnow"
    cursor.execute(sql)
    result = cursor.fetchall()
    cursor.close()
    ...
    for i in range(0, len(centroid) - 1):
        for j in range(i + 1, len(centroid)):
            ...
            distances.append(list(zip(centroid[i][:], centroid[j][:], itertools.repeat(dist))))

    print(distances)
    length = len(distances)

    # distances = defaultdict(list)
    # print("im dict list", distances.items())


    return render_template('question8.html', ci=distances, totalPoints = length)
